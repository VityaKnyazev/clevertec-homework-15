...
public class PersonServiceImplTestData {
    private static final Long EXPECTED_PERSON_ID = 1L;
    private static final String EXPECTED_AND_INPUT_PERSON_UUID = "8d113b5a-59d5-4f60-839b-506309d6d9f0";
    private static final String EXPECTED_AND_INPUT_PERSON_NAME = "Nikolay";
    private static final String EXPECTED_AND_INPUT_PERSON_SURNAME = "Klimov";
    private static final PersonSex EXPECTED_AND_INPUT_PERSON_SEX = PersonSex.male;
    ...
    public static Person expectedPerson() {
        ...
        House expectedLivingHouse = HouseServiceImplTestData.expectedHouse();
        List<House> expectedPossessingHouses = HouseServiceImplTestData.expectedPageHouses()
                .stream().collect(Collectors.toList());

        return Person.builder()
                .id(EXPECTED_PERSON_ID)
                .uuid(UUID.fromString(EXPECTED_AND_INPUT_PERSON_UUID))
                .name(EXPECTED_AND_INPUT_PERSON_NAME)
                .surname(EXPECTED_AND_INPUT_PERSON_SURNAME)
                .sex(EXPECTED_AND_INPUT_PERSON_SEX)
    ...
    public static Page<Person> expectedPagePersons() {
        Passport expectedPassport = PassportServiceImplTestData.expectedPassport();
        House expectedLivingHouse = HouseServiceImplTestData.expectedHouse();
        List<House> expectedPossessingHouses = HouseServiceImplTestData.expectedPageHouses().getContent();
    ...
    public static PostPutPersonRequestDTO inputPostPersonRequestDTO() {
        return PostPutPersonRequestDTO.builder()
                .name(EXPECTED_AND_INPUT_PERSON_NAME)
                .surname(EXPECTED_AND_INPUT_PERSON_SURNAME)
                .sex(EXPECTED_AND_INPUT_PERSON_SEX.name())
    ...
    public static PostPutPersonRequestDTO inputPutPersonRequestDTO() {
        return PostPutPersonRequestDTO.builder()
                .uuid(EXPECTED_AND_INPUT_PERSON_UUID)
                .name(EXPECTED_AND_INPUT_PERSON_NAME)
                .surname(EXPECTED_AND_INPUT_PERSON_SURNAME)
                .sex(EXPECTED_AND_INPUT_PERSON_SEX.name())
                .passportUUID(PassportServiceImplTestData.REPLACEABLE_PASSPORT_UUID)
                .livingHouseUUID(HouseServiceImplTestData.REPLACEABLE_HOUSE_UUID)
                .possessingHouseUUIDs(List.of(HouseServiceImplTestData.REPLACEABLE_HOUSE_UUID))
                .build();
    }
}
